...
import {
  ...
  CANONICAL_VENDOR_FIELDS,
  type CanonicalVendorFieldKey,
  emptyVendorUploadMapping,
  newEntryId,
  type ParsedVendorUpload,
  type VendorCatalogCreate,
  type VendorCatalogDefault,
  type VendorCatalogEntry,
  type VendorCatalogSource,
  type VendorUploadMapping,
} from "@/types/vendor-catalog";

/**
 * "New vendor catalog" dialog — a two-step wizard.
 *
 * Step 1 — SETUP (size=md):
 *   * Name + description
 *   * Pick a starter mode: default / from_upload (a "from blank" mode
 *     is intentionally NOT offered — vendor catalogs always start from
 *     either the canonical default or a real source file, so the saved
 *     catalog matches BillsIQ's canonical structure from row one).
 ...
 *   * One <select> per canonical BillsIQ vendor field, populated with
 *     the uploaded file's source columns + a "— None —" sentinel.
 *   * Pre-seeded from `parsed.suggested_mapping`. Always shown to the
 *     user — never silently committed. Vendor Name is the only
 *     required field and gates the Create button.
 *   * Live preview of the first 5 mapped rows (vendor_name +
 *     vendor_code + aliases + city/state — the "headline" columns;
 *     the editor handles the rest), plus counts of skipped blank-
 *     required rows and de-duplicated codes.
 ...
 *
 *   The uploaded file's column names can be anything — vendor masters
 *   ship with names like "Supplier ID", "Payee Name", "DBA",
 *   "Remit-to". BillsIQ's canonical vendor schema is the source of
 *   truth (vendor_name / vendor_code / aliases / address / city /
 *   state / zip / contact_name / email / phone / active / notes).
 *   Auto-mapping headers and silently building entries from them would
 *   hide misdetections from the user — exactly the failure mode this
 *   wizard is designed to prevent. The mapping step makes the source →
 *   canonical mapping explicit and overridable.
 *
 * Session-local upload state:
 *
 *   The modal is session-local. There is NO persistent "last uploaded
 *   vendor master" slot in the system — uploads are one-shot parses by
 ...
 *   never carries over.
 */
interface NewVendorCatalogModalProps {
  open: boolean;
  saving: boolean;
  error: string | null;
  ...
  onClose: () => void;
  onCreate: (body: VendorCatalogCreate) => Promise<void>;
}

// Note: "blank" is deliberately NOT a valid start mode for vendor
// catalogs. Vendor catalogs must always start from either the
// canonical default or a real uploaded vendor master, so the saved
// catalog conforms to BillsIQ's canonical vendor structure from
// the first row. Already-saved catalogs with `source = "blank"`
// (created before this restriction) keep working — only the
// CREATION path is restricted.
...
export function NewVendorCatalogModal({
  open,
  saving,
  error,
  defaultCatalog,
  ...
  const [sessionUpload, setSessionUpload] =
    useState<ParsedVendorUpload | null>(null);
  ...
  const [mapping, setMapping] = useState<VendorUploadMapping>(
    emptyVendorUploadMapping,
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      setUploadProgress(0);
      setUploadError(null);
      setMapping(emptyVendorUploadMapping());
    }
  }, [open]);
  ...
  const requiredFieldsMapped =
    mapping.vendor_name != null && mapping.vendor_name.length > 0;
  ...
  const buildDirectEntries = (): VendorCatalogEntry[] => {
    ...
    if (mode === "default" && defaultCatalog) {
      return defaultCatalog.entries.map((e) => ({
        ...
        id: newEntryId(),
        // Defensive copy of the alias array so the draft can mutate
        // its own list without leaking into the cached default.
        aliases: [...e.aliases],
      }));
    }
    return [];
  ...
  const handleConfirmMapping = async () => {
    ...
    await onCreate({
      ...
      source: "from_upload" as VendorCatalogSource,
    });
  };
  ...
  const handleFileChosen = async (
    ...
    setUploadError(null);
    try {
      const parsed = await vendorCatalogsApi.parseUpload(file, (pct) =>
...
interface SetupStepProps {
  ...
  mode: StartMode | null;
  defaultCatalog: VendorCatalogDefault | null;
  sessionUpload: ParsedVendorUpload | null;
  uploading: boolean;
...
function SetupStep({
  ...
  return (
    <div className="space-y-3">
      <div>
        <FieldLabel htmlFor="new-vendor-name">Name</FieldLabel>
        <input
          id="new-vendor-name"
          type="text"
          value={name}
          ...
          placeholder="e.g. Active Vendors 2026"
      ...
      <div>
        <FieldLabel htmlFor="new-vendor-desc">Description</FieldLabel>
        <textarea
          id="new-vendor-desc"
          value={description}
          rows={2}
      ...
      <div>
        ...
        <div className="grid gap-1.5">
          <StartOption
            ...
            description={
              defaultCatalog
                ? `Start with ${defaultCatalog.entries.length} sensible multifamily vendor entries. Edit, add, or remove rows after creating.`
          ...
          {/* "From uploaded file" — option button + a sub-card that
              ...
              state with an "Upload a file" CTA. */}
          <div className="space-y-1.5">
            <StartOption
              icon={Upload}
              title="From uploaded vendor master"
              description={
                sessionUploadHasColumns
                  ? `Use the ${sessionUpload!.source_rows.length}-row file you just uploaded. Map its columns to BillsIQ's vendor fields next.`
                  : "Upload your existing vendor master now (CSV / Excel) — we'll surface its columns so you can map them to BillsIQ's vendor fields next."
              }
...
// ===========================================================================
// Step 1 sub-card — session-local upload state for "From uploaded file"
// ===========================================================================
...
function UploadedSourceCard({
  ...
  parsed: ParsedVendorUpload | null;
  uploading: boolean;
  progress: number;
  uploadError: string | null;
  onUploadClick: () => void;
      ...
      {uploading ? (
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[10.5px] text-gray-600 mt-1 dark:text-ink-muted">
            ...
            match these columns to BillsIQ&apos;s vendor fields.
          </p>
        ...
        <p className="text-gray-600 dark:text-ink-muted">
          No file picked yet. Upload a vendor master (CSV or Excel)
  ...
  return (
    <div className="space-y-4">
      <p className="text-[12.5px] text-gray-600 dark:text-ink-muted">
        Match each BillsIQ vendor field to a column from{" "}
      ...
      {/* ---- Preview ---------------------------------------------------- */}
      {/* Vendor entries have 12 fields — cramming all of them into a
          modal-width preview makes every column unreadable. So we
          surface the headline columns (name + code + aliases + city/
          state) in the preview and trust the editor to expose the
          full row on the next screen. */}
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <p className="text-[10.5px] text-gray-500 dark:text-ink-muted">
            {requiredFieldsMapped ? (
              ...
              <>
                Pick a source column for the required field above to see
                the preview.
              </>
            )}
          </p>
        ...
        {requiredFieldsMapped && previewRows.length > 0 ? (
          <div className="overflow-x-auto rounded-md border border-gray-200 dark:border-line">
            <table className="w-full text-[11.5px]">
              <thead className="bg-gray-50 border-b border-gray-200 dark:bg-surface-muted dark:border-line">
                <tr>
                  <th className="text-left font-semibold text-gray-600 px-2.5 py-1.5 dark:text-ink-muted">
                    Vendor name
                  </th>
                  <th className="text-left font-semibold text-gray-600 px-2.5 py-1.5 dark:text-ink-muted">
                    Code
              ...
              <tbody>
                {previewRows.map((entry) => (
                  ...
                  >
                    <td className="px-2.5 py-1.5 font-medium text-gray-800 dark:text-ink">
                      {entry.vendor_name}
                    </td>
                    <td className="px-2.5 py-1.5 font-mono text-gray-700 dark:text-ink-muted">
                      {entry.vendor_code ?? (
                        <span className="text-gray-300 dark:text-ink-subtle">—</span>
                      )}
                    </td>
                    <td className="px-2.5 py-1.5 text-gray-600 dark:text-ink-muted max-w-[14rem] truncate">
          ...
          <div className="rounded-md border border-dashed border-gray-300 px-3 py-6 text-center text-[11.5px] text-gray-500 dark:border-line dark:text-ink-muted">
            {requiredFieldsMapped
              ...
              : "Preview unavailable until the required field is mapped."}
          </div>
        )}
        {requiredFieldsMapped && previewRows.length > 0 && (
          <p className="text-[10px] text-gray-400 mt-1.5 dark:text-ink-subtle">
            Address, contact, and notes columns aren&apos;t shown here to
            keep the preview readable — they&apos;re carried into the
            editor on the next screen.
          </p>
        )}
      </div>
      ...
      <ul className="space-y-1 text-[11.5px]">
        ...
        {blankRequiredCount > 0 && (
          <li className="text-yellow-700 dark:text-yellow-200">
            ...
            {blankRequiredCount === 1 ? "" : "s"} with a blank vendor name.
          </li>
        )}
        {duplicateCodeCount > 0 && (
          <li className="text-yellow-700 dark:text-yellow-200">
            Dropping {duplicateCodeCount} duplicate vendor code
            {duplicateCodeCount === 1 ? "" : "s"} (first occurrence wins;
            rows without a code are kept).
          </li>
        )}
...
// ===========================================================================
// Start-mode option card (visually identical to NewGLCatalogModal's)
// ===========================================================================
