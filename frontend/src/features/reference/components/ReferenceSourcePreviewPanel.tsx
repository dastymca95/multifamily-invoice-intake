...
export function ReferenceSourcePreviewPanel({
  ...
  return (
    <aside className="h-full w-full bg-white border border-gray-200 rounded-xl flex flex-col min-h-0 min-w-0">
    ...
    <div className="px-4 py-3 space-y-3">
      ...
      <div className="flex items-start gap-2">
        <div
          className={cn(
            ...
            parseFailed ? "bg-yellow-100" : "bg-brand-50",
          )}
        >
          <Icon
            className={cn(
              "h-4 w-4",
              parseFailed ? "text-yellow-700" : "text-brand-700",
            )}
          />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-[10.5px] uppercase tracking-wide text-gray-400 font-semibold">
            {slot.label}
          </p>
          <p
            className="text-[12.5px] font-semibold text-gray-800 truncate"
      ...
      {parseFailed ? (
        ...
        <>
          ...
          <p className="text-[11.5px] text-gray-700 inline-flex items-center gap-1.5">
          ...
          {keyFields.length > 0 && (
            <div className="rounded-md border border-gray-200 bg-gray-50/80 px-2.5 py-1.5">
              <p className="text-[10px] uppercase tracking-wide text-gray-400 font-semibold inline-flex items-center gap-1 mb-1">
          ...
          {isTemplate ? (
            ...
            <p className="text-[11px] text-gray-500 inline-flex items-start gap-1.5">
              <AlertTriangle className="h-3 w-3 text-gray-400 shrink-0 mt-0.5" />
              <span>
...
function NormalizedTable({
  ...
  return (
    <div>
      ...
      <div className="relative overflow-auto rounded-md border border-gray-200">
        <table className="min-w-full text-[11px] border-collapse">
          <thead className="sticky top-0 z-20 bg-gray-50">
            <tr>
              <th className="sticky left-0 z-30 bg-gray-100 border-r border-b border-gray-200 px-2 py-1.5 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wide w-[3rem] min-w-[3rem]">
                #
              </th>
              {columns.map((col, i) => {
                ...
                return (
                  <th
                    ...
                    className={cn(
                      ...
                      isKey ? "bg-brand-50/70" : "bg-gray-50",
                    )}
                    title={col}
                  >
                    <div className="flex flex-col gap-0.5">
                      <span
                        className={cn(
                          ...
                          isKey ? "text-brand-900" : "text-gray-800",
                        )}
                      >
                        {prettyHeader(col)}
                      </span>
                      <span
                        className={cn(
                          ...
                          isKey ? "text-brand-700/80" : "text-gray-400",
                        )}
                      >
                        {col}
                      </span>
            ...
            {rows.length === 0 ? (
              <tr>
                <td
                  ...
                  className="px-3 py-4 text-center text-[11px] text-gray-500 italic bg-white"
                >
              ...
              rows.map((row, ri) => {
                const stripeClass = ri % 2 === 0 ? "bg-white" : "bg-gray-50/40";
                return (
                  <tr key={ri} className={stripeClass}>
                    <th
                      className={cn(
                        "sticky left-0 z-10 border-r border-b border-gray-200 px-2 py-1 text-left align-top text-[10px] font-mono text-gray-400 w-[3rem] min-w-[3rem]",
                        stripeClass,
                      )}
                    >
                      {ri + 1}
                    ...
                    {columns.map((col, ci) => {
                      ...
                      return (
                        <td
                          key={`${ci}-${col}`}
                          className={cn(
                            "border-r border-b border-gray-200 px-2 py-1 align-top whitespace-nowrap max-w-[16rem] truncate",
                            isEmpty ? "text-gray-300 italic" : "text-gray-700",
                            isKey && !isEmpty && "font-medium text-gray-900",
                          )}
                          title={
                            isEmpty
